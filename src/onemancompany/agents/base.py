...
class GatewayChatModel(BaseChatModel):
    ...
    The wrapped runnable may be either a raw chat model or the RunnableBinding
    returned by ``bind_tools``. Keeping the gateway at this layer means every
    reasoning turn in a ReAct graph is limited, rather than merely limiting the
    duration of the whole agent task.
    """
    ...
    def _llm_type(self) -> str:
        return "onemancompany-provider-gateway"

    @property
    def _identifying_params(self) -> dict[str, Any]:
        return {
            "provider": self.provider_context.get("provider", "default"),
            "model": self.provider_context.get("account_or_model_pool", "default"),
        }
    ...
    def bind_tools(
        ...
        **kwargs: Any,
        ...
        if not hasattr(self.delegate, "bind_tools"):
            raise TypeError("Wrapped model does not support tool binding")
        bound = self.delegate.bind_tools(tools, tool_choice=tool_choice, **kwargs)
        return self.model_copy(update={"delegate": bound})
    ...
    async def _agenerate(
        ...
        **kwargs: Any,
        ...
        from onemancompany.core.provider_gateway import ProviderPriority, get_provider_gateway
        from onemancompany.core.runtime_context import get_task_runtime_context

        async def _invoke():
            response = await self.delegate.ainvoke(messages, stop=stop, **kwargs)
            if not isinstance(response, BaseMessage):
                raise TypeError(f"Chat model returned unsupported result: {type(response)!r}")
            return response
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        # Server-side agents are async. Synchronous invocation remains available
        # only before the runtime gateway starts (primarily import/unit tests).
        from onemancompany.core.provider_gateway import get_provider_gateway

        if get_provider_gateway() is not None:
            raise RuntimeError("GatewayChatModel requires async invocation while ProviderGateway is active")
...
def _annotate_provider_model(
    llm: BaseChatModel, *, provider: str, credential: str, model: str
) -> BaseChatModel:
    """Attach non-secret scheduling metadata without changing the public model type."""
    from onemancompany.core.provider_gateway import credential_fingerprint

    context = {
        "provider": provider or "default",
        "credential_fingerprint": credential_fingerprint(credential),
        "account_or_model_pool": model or "default",
    }
    try:
        object.__setattr__(llm, "_omc_provider_context", context)
    except Exception as exc:
        logger.debug("Unable to attach provider scheduling metadata: {}", type(exc).__name__)
    return llm


def wrap_llm_for_runtime(
    llm: BaseChatModel, *, priority: int = 0
) -> BaseChatModel:
    """Wrap a model only when the runtime ProviderGateway is active."""
    from onemancompany.core.provider_gateway import get_provider_gateway

    if get_provider_gateway() is None or isinstance(llm, GatewayChatModel):
        return llm
    context = getattr(llm, "_omc_provider_context", None) or {
        "provider": "default",
        "credential_fingerprint": "anonymous",
        "account_or_model_pool": getattr(llm, "model_name", None) or getattr(llm, "model", "default"),
    }
    return GatewayChatModel(delegate=llm, provider_context=dict(context), priority=priority)


def create_runtime_react_agent(*, model: BaseChatModel, tools, **kwargs):
    """Compile a ReAct agent with official runtime Saver/Store when available."""
    from onemancompany.core.runtime_storage import get_runtime_storage

    storage = get_runtime_storage()
    if storage is not None:
        kwargs.setdefault("checkpointer", storage.checkpointer)
        kwargs.setdefault("store", storage.memory_store)
    return _langgraph_prebuilt.create_react_agent(
        model=wrap_llm_for_runtime(model), tools=tools, **kwargs
    )


def create_react_agent(*, model: BaseChatModel, tools, **kwargs):
    """Backward-compatible factory alias used by legacy callers and tests.

    New runtime code should call :func:`create_runtime_react_agent`; keeping
    this stable symbol avoids breaking integrations that monkeypatch the
    historical factory name.
    """
    return create_runtime_react_agent(model=model, tools=tools, **kwargs)
...
def make_llm(employee_id: str = "", temperature: float | None = None) -> BaseChatModel:
    ...
    if prov and effective_chat_class == CHAT_CLASS_ANTHROPIC:
        ...
        if effective_key:
            ...
            return _annotate_provider_model(ChatAnthropic(
                model=model,
            ...
            ), provider=api_provider, credential=effective_key, model=model)
    ...
    if prov and effective_chat_class == CHAT_CLASS_OPENAI:
        ...
        if effective_key:
            ...
            return _annotate_provider_model(ChatOpenAI(
                model=model,
                api_key=effective_key,
                base_url=base_url,
                temperature=effective_temp,
    ...
    return _annotate_provider_model(ChatOpenAI(
        ...
        request_timeout=300.0,
        stream_usage=True,
    ), provider="openrouter", credential=fallback_key, model=model)
...
async def tracked_ainvoke(
    ...
    *,
    ...
    if gateway is None:
        ...
        context = dict(getattr(llm, "_omc_provider_context", {}) or {})
        context.update(get_task_runtime_context())
        priority = ProviderPriority.REVIEW if category in {"review", "meeting"} else ProviderPriority.BUSINESS
        result = await gateway.invoke(
            context=context, priority=priority, invoke=lambda: llm.ainvoke(messages)
        )
...
class BaseAgentRunner:
    ...
    @staticmethod
    def _sanitize_persisted_input(value: str) -> str:
        """Redact common credentials before model input is persisted in checkpoints/logs."""
        from onemancompany.core.memory_service import redact_sensitive
        return redact_sensitive(value)

    def _checkpoint_messages(self, prompt: str, task: str):
        return [
            SystemMessage(content=self._sanitize_persisted_input(prompt)),
            HumanMessage(content=self._sanitize_persisted_input(task)),
        ]

    def _checkpoint_input(self, prompt: str, task: str):
        from onemancompany.core.runtime_context import get_task_runtime_context
        if get_task_runtime_context().get("checkpoint_resume"):
            return None
        return {"messages": self._checkpoint_messages(prompt, task)}

    async def _inject_long_term_memory(self, prompt: str, task: str) -> str:
        """Inject bounded, ACL-filtered memory only for formal runtime tasks."""
        from onemancompany.core.runtime_context import get_task_runtime_context
        from onemancompany.core.runtime_storage import get_runtime_storage
        ...
        storage = get_runtime_storage()
        if (
            not settings.omc_memory_enabled
            or storage is None
            or storage.memory_store is None
            or context.get("memory_injected")
        ):
            return prompt
        try:
            from onemancompany.core.memory_service import MemoryService
            model_budget_chars = max(0, int(context.get("model_input_budget_chars") or 30000))
            memory_budget_chars = min(6000, int(model_budget_chars * 0.20))
            rows = await MemoryService(storage).search(
                employee_id=self.employee_id, query=task,
                project_id=str(context.get("project_id") or ""), limit=8,
            ...
            for row in rows:
                rendered.append(
                    "- memory_id={memory_id}; scope={scope}; status={status}; "
                    "source_node_id={source_node_id}; verified_at={verified_at}; "
            ...
            return prompt + "\n\n## Retrieved Long-term Memory (context only)\n" + "\n".join(rendered)
        except Exception as exc:
            logger.warning("Long-term memory retrieval degraded for {}: {}", self.employee_id, type(exc).__name__)
            return prompt
    ...
    async def run_streamed(self, task: str, on_log=None) -> str:
        ...
        prompt = await self._inject_long_term_memory(prompt, task)
        from onemancompany.core.runtime_context import get_task_runtime_context
        _resume_checkpoint = bool(get_task_runtime_context().get("checkpoint_resume"))
        # Formal recovery must continue the persisted graph state. Re-appending
        # the original system/user messages would duplicate the task and can
        # replay a side effect. ``None`` is the LangGraph resume input.
        initial_msgs = self._checkpoint_messages(prompt, task)
        messages_input = None if _resume_checkpoint else {"messages": initial_msgs}
        ...
        try:
            async for event in self._agent.astream_events(
                messages_input, version="v2",
                config=__import__("onemancompany.core.runtime_context", fromlist=["langgraph_invoke_config"]).langgraph_invoke_config(recursion_limit=50),
            ):
...
class EmployeeAgent(BaseAgentRunner):
    ...
    async def run(self, task: str) -> str:
        ...
        prompt = self._build_full_prompt()
        prompt = await self._inject_long_term_memory(prompt, task)
        from onemancompany.core.runtime_context import get_task_runtime_context, langgraph_invoke_config
        initial_msgs = self._checkpoint_messages(prompt, task)
        _resume_checkpoint = bool(get_task_runtime_context().get("checkpoint_resume"))
        result = await self._agent.ainvoke(
            None if _resume_checkpoint else {"messages": initial_msgs},
            config=langgraph_invoke_config(),
        )
