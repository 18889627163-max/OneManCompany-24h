...
# ---------------------------------------------------------------------------
from onemancompany.core.config import DATA_ROOT, ENCODING_UTF8, ENV_OMC_DEBUG, LAUNCH_SH_FILENAME, LogLevel, DATA_DIR_NAME, DOT_ENV_FILENAME, PF_LEVEL, PF_REMOTE, SRC_DIR_NAME, SYSTEM_AGENT
...
_log_dir = DATA_ROOT / LOG_DIR_NAME
...
def _restore_persisted_tasks_on_startup(manager, *, enabled: bool) -> int:
    """Restore and resume authoritative TaskTree work when explicitly enabled.

    Keeping this behind a dedicated startup fence lets health, backup and
    restore verification boot the real service without mutating production
    TaskTree files. It does not change the default production behaviour.
    """
    if not enabled:
        logger.info("Persisted TaskTree recovery disabled by OMC_RESTORE_PERSISTED_TASKS")
        return 0
    restored_count = manager.restore_persisted_tasks()
    if restored_count:
...
async def _application_lifespan(app: FastAPI):
    # Keep the application and runtime lifespans on the same settings singleton.
    ...
    # Reconcile formal TaskTree state with durable LangGraph checkpoints before
    # rebuilding schedules. Isolated service verification disables both through
    # OMC_RESTORE_PERSISTED_TASKS=false, so authoritative trees are untouched.
    app.state.checkpoint_reconciliation = None
    if settings.omc_restore_persisted_tasks:
        _runtime_storage = getattr(app.state, "runtime_storage", None)
        if _runtime_storage is not None:
            try:
                from onemancompany.core.checkpoint_reconciler import reconcile_checkpoints
                from onemancompany.core.config import PROJECTS_DIR

                app.state.checkpoint_reconciliation = await reconcile_checkpoints(
                    _runtime_storage, PROJECTS_DIR
                )
            except Exception as exc:
                logger.exception("Checkpoint reconciliation failed closed: {}", type(exc).__name__)
                app.state.checkpoint_reconciliation = None

    # Restore persisted tasks from per-employee task files. This is fenced so
    # real-service verification can run without touching authoritative trees.
    from onemancompany.core.vessel import employee_manager as _em
    _restore_persisted_tasks_on_startup(
        _em, enabled=settings.omc_restore_persisted_tasks
    )

    # The standard-v2 automation manifest starts only after every employee
    # executor and persisted schedule has been restored.  Registration remains
    # durable in SQLite; startup failure degrades automation without disabling
    # the otherwise healthy runtime/checkpoint service.
    _runtime_storage = getattr(app.state, "runtime_storage", None)
    app.state.automation_registration = None
    app.state.automation_manifest_runner = False
    if _runtime_storage is not None:
        try:
            from onemancompany.core.automation_manifest import register_manifest, start_manifest_automations
            # Registration is durable even when execution is disabled. This
            # makes health/verify able to prove the manifest without scheduling
            # business work in an isolated environment.
            registration = await register_manifest(_runtime_storage)
            app.state.automation_registration = registration
            if settings.omc_automation_enabled:
                await start_manifest_automations(_runtime_storage)
                app.state.automation_manifest_runner = True
            else:
                logger.info("Standard automation runner disabled by OMC_AUTOMATION_ENABLED")
        except Exception as exc:
            app.state.automation_registration = None
            app.state.automation_manifest_runner = False
            logger.exception("Standard automation manifest unavailable: {}", type(exc).__name__)
    ...
    # Stop standard manifest scheduling before shutting down employee loops so
    # no new task can be registered during teardown.
    from onemancompany.core.automation_manifest import stop_manifest_automations
    try:
        await stop_manifest_automations()
    except asyncio.CancelledError:
        logger.debug("Standard automation runner cancelled during shutdown")
    app.state.automation_manifest_runner = False

    # Stop producers before employee loops. System crons can enqueue work and
    # completion callbacks, so stopping loops first allowed a late callback to
    # recreate EmployeeManager's consumer during event-loop teardown.
    await _system_cron_mod.system_cron_manager.stop_all()

    # Stop agent loops after every in-process scheduler has been fenced.
...
async def _prepare_memory_index(settings):
    """Build the optional SQLite vector index without blocking core execution.

    A missing/unreachable embedding endpoint degrades memory retrieval to the
    structured store.  Only a configured index contract mismatch is allowed to
    fail runtime initialization, because writing into a different vector space
    would corrupt retrieval semantics.
    """
    if not settings.omc_memory_enabled:
        return None, "disabled", "disabled"

    required = (
        settings.omc_memory_embedding_base_url,
        settings.omc_memory_embedding_api_key,
        settings.omc_memory_embedding_model,
        settings.omc_memory_embedding_dimensions,
    )
    if not all(required):
        logger.warning("Memory embedding configuration incomplete; using structured-only memory")
        return None, "degraded", "unavailable"

    from langchain_openai import OpenAIEmbeddings

    embeddings = OpenAIEmbeddings(
        base_url=settings.omc_memory_embedding_base_url,
...
async def _runtime_lifespan(app: FastAPI):
    ...
    gateway = None
    memory_worker = None
    app.state.memory_embedding_status = "disabled" if not settings.omc_memory_enabled else "degraded"
    app.state.memory_vector_status = "disabled" if not settings.omc_memory_enabled else "unavailable"
    try:
        ...
        try:
            concurrency = max(1, int(os.environ.get("OMC_PROVIDER_CONCURRENCY", "1")))
        except ValueError:
            logger.warning("Invalid OMC_PROVIDER_CONCURRENCY; using safe default 1")
            concurrency = 1
        gateway = ProviderGateway(storage, default_concurrency=concurrency)
        await gateway.start()
        ...
        # Registration/runner startup happens in _application_lifespan after all
        # employees and executors are registered.  Starting here would race the
        # EmployeeManager and could persist false holding receipts.
        app.state.automation_registration = None
        app.state.automation_manifest_runner = False
        app.state.runtime_storage = storage
        app.state.provider_gateway = gateway
        app.state.memory_worker = memory_worker
        app.state.runtime_storage_error = None
        logger.info("Runtime SQLite and ProviderGateway initialized at {}", storage.db_path)
    except Exception as exc:
        # Keep legacy/simple workflows available, but formal standard-v2 execution
        # is rejected at the executor boundary when runtime storage is absent.
        logger.exception("Runtime persistence unavailable; standard v2 execution is disabled")
        ...
        app.state.automation_registration = None
        app.state.automation_manifest_runner = False
        app.state.runtime_storage_error = type(exc).__name__
        if settings.omc_memory_enabled:
    ...
    try:
        ...
        if gateway is not None:
            await gateway.stop()
        set_provider_gateway(None)
        if app.state.runtime_storage is not None:
            await storage.close()
        set_runtime_storage(None)
        app.state.provider_gateway = None
        app.state.runtime_storage = None
