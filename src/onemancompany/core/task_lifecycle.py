...
VALID_TRANSITIONS: dict[TaskPhase, list[TaskPhase]] = {
    ...
    TaskPhase.PROCESSING: [TaskPhase.COMPLETED, TaskPhase.HOLDING, TaskPhase.FAILED, TaskPhase.BLOCKED, TaskPhase.CANCELLED],
...
# Workflow contract v2 gate policies.  These are allow-lists on purpose:
# malformed or future statuses must fail closed instead of silently passing.
PARENT_VALID = frozenset({
    TaskPhase.PROCESSING, TaskPhase.HOLDING, TaskPhase.COMPLETED,
    TaskPhase.ACCEPTED, TaskPhase.FINISHED,
})
PARENT_TERMINAL_FAILURE = frozenset({
    TaskPhase.FAILED, TaskPhase.BLOCKED, TaskPhase.CANCELLED,
})
CLOSURE_COMPLETE = frozenset({TaskPhase.ACCEPTED, TaskPhase.FINISHED})


def parse_task_phase(raw_status: str | TaskPhase | None) -> TaskPhase:
    """Parse a persisted status strictly, failing closed on missing/invalid values."""
    if isinstance(raw_status, TaskPhase):
        return raw_status
    if not isinstance(raw_status, str) or not raw_status:
        raise ValueError("task status is empty")
    return TaskPhase(raw_status)
