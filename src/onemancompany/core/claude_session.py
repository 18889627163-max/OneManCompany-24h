...
# Claude CLI emits each stream-json event as one NDJSON line. Tool results can
# legitimately exceed asyncio's 64 KiB subprocess StreamReader default, which
# otherwise raises LimitOverrunError (surfaced as the misleading separator/chunk
# daemon errors). Keep a bounded but practical per-line transport limit.
CLAUDE_STREAM_READER_LIMIT = 16 * 1024 * 1024
...
def write_llm_trace(project_id: str, entry: dict) -> None:
    ...
    try:
        ...
        safe_entry = redact_sensitive_value(entry)
    ...
    async def start(self) -> None:
        ...
        self.proc = await asyncio.create_subprocess_exec(
            *cmd,
            ...
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE,
            limit=CLAUDE_STREAM_READER_LIMIT,
            cwd=self.work_dir,
            env=env,
        )
