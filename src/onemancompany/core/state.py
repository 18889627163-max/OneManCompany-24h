...
class TaskEntry:
    ...
    project_id: str        # base project ID (without /iter_XXX suffix)
    task: str
    routed_to: str = ""    # "HR", "COO", "EA", etc.
    iteration_id: str = ""  # v2 = iter_XXX, v1 = empty
    node_id: str = ""       # durable TaskNode ID
    parent_id: str = ""     # direct parent TaskNode ID
    tree_path: str = ""     # absolute task_tree.yaml path
...
def get_active_tasks() -> list[TaskEntry]:
    """Build the durable active-task projection from queued and running entries.

    ``_schedule`` contains queued work while ``_current_entries`` contains the
    entry currently handed to an employee runner.  They can overlap briefly at
    the launch boundary, so merge both sources and deduplicate by the durable
    ``(tree_path, node_id)`` identity.  The TaskNode status remains the source
    of truth for whether an entry is active.
    """
    ...
    entries_by_employee: dict[str, list] = {}
    for employee_id, entries in employee_manager._schedule.items():
        entries_by_employee.setdefault(employee_id, []).extend(entries)
    for employee_id, entry in getattr(employee_manager, "_current_entries", {}).items():
        if entry is not None:
            entries_by_employee.setdefault(employee_id, []).append(entry)

    seen: set[tuple[str, str]] = set()
    active_statuses = {
        TaskPhase.PENDING.value,
        TaskPhase.PROCESSING.value,
        TaskPhase.HOLDING.value,
        TaskPhase.BLOCKED.value,
    }
    for employee_id, entries in entries_by_employee.items():
        for entry in entries:
            tp = Path(entry.tree_path)
            identity = (str(tp.resolve()), str(entry.node_id))
            if identity in seen or not tp.exists():
                continue
            try:
                tree = TaskTree.load(tp)
                node = tree.get_node(entry.node_id)
                if not node or node.status not in active_statuses:
                    continue
                node.load_content(tp.parent)
                seen.add(identity)
                raw_project_id = str(node.project_id or "")
                base_project_id, separator, iteration_id = raw_project_id.partition("/")
                if not separator:
                    base_project_id = raw_project_id
                    iteration_id = tp.parent.name if tp.parent.name.startswith("iter_") else ""
                result.append(TaskEntry(
                    project_id=base_project_id,
                    iteration_id=iteration_id,
                    node_id=node.id,
                    parent_id=node.parent_id,
                    tree_path=str(tp.resolve()),
                    task=node.description,
                    project_dir=node.project_dir or str(tp.parent),
    ...
    for emp_dir in EMPLOYEES_DIR.iterdir():
        if emp_dir.is_dir():
            try:
                ...
                logger.debug("Skipping non-numeric employee directory: {}", emp_dir.name)
                continue
