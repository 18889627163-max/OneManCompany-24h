...
# Data root — all runtime/company data lives under cwd/.onemancompany/ by
# default. OMC_DATA_ROOT provides an explicit process-level isolation boundary
# for verification, maintenance and independent recovery drills. It must be set
# in the process environment before importing OneManCompany configuration.
DATA_DIR_NAME = ".onemancompany"
_data_root_override = os.environ.get("OMC_DATA_ROOT", "").strip()
DATA_ROOT = Path(_data_root_override).expanduser() if _data_root_override else Path.cwd() / DATA_DIR_NAME
...
class Settings(BaseSettings):
    ...
    # SQLite-backed long-term memory (single-host standard v2)
    omc_memory_enabled: bool = False
    # Durable standard-v2 automation is independently switchable so isolated
    # health/restore verification cannot enqueue business work.
    omc_automation_enabled: bool = True
    # Startup recovery is separately switchable. Verification and maintenance
    # processes must be able to initialize the application without scanning or
    # resuming the authoritative TaskTree under .onemancompany/.
    omc_restore_persisted_tasks: bool = True
    omc_memory_database_path: str = ".onemancompany/data/runtime.sqlite3"
    omc_memory_embedding_base_url: str = ""
    omc_memory_embedding_api_key: str = ""
    omc_memory_embedding_model: str = ""
    omc_memory_embedding_dimensions: int | None = None
    omc_memory_index_version: str = "v1"

    # Loopback-only runtime administration
    omc_admin_token: str = ""
    omc_runtime_backup_dir: str = "backups/db"

    @field_validator("omc_memory_embedding_dimensions", mode="before")
    @classmethod
    def _empty_embedding_dimensions_as_none(cls, value):
        """Allow the documented empty .env value to mean structured-only memory."""
...
def load_validated_employee_profile(
    profile_path: Path,
    *,
    employee_id: str = "",
    profile_kind: str = "employee",
) -> dict | None:
    """Safely load and validate one employee profile.

    Returns the original mapping so runtime/extension fields remain available to
    API callers. Invalid YAML, unsafe Python tags, non-mappings, and profiles
    missing the required ``name``/``role``/``skills`` contract are skipped.
    """
    try:
        ...
        EmployeeConfig.model_validate(raw)
        return raw
    except Exception as exc:
        logger.warning(
            "[EMPLOYEE_PROFILE_INVALID] kind={} employee_id={} path={} error={}",
            profile_kind, employee_id or profile_path.parent.name, profile_path, exc,
        )
        return None
...
def load_employee_configs() -> dict[str, EmployeeConfig]:
    ...
    for emp_dir in sorted(EMPLOYEES_DIR.iterdir()):
        ...
        if not profile_path.exists():
            continue
        emp_id = emp_dir.name
        raw = load_validated_employee_profile(
            profile_path, employee_id=emp_id, profile_kind="employee",
        )
        if raw is None:
            continue
        result[emp_id] = EmployeeConfig.model_validate(raw)
    return result
...
def load_ex_employee_configs() -> dict[str, EmployeeConfig]:
    ...
    for emp_dir in sorted(EX_EMPLOYEES_DIR.iterdir()):
        ...
        emp_id = emp_dir.name
        raw = load_validated_employee_profile(
            profile_path, employee_id=emp_id, profile_kind="ex_employee",
        )
        if raw is None:
            continue
        result[emp_id] = EmployeeConfig.model_validate(raw)
