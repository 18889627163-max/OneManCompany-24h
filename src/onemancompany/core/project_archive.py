...
def create_iteration(
    ...
    *,
    ...
    """Create a new iteration under an existing named project.

    ``project_dir`` remains the internal OneManCompany iteration workspace.
    ``implementation_path`` is a separate, durable pointer to the authoritative
    business repository.  Keeping the two paths distinct prevents an iteration
    workspace from silently being treated as the repository to modify.
    """
    ...
    # Pick an ID that is free both in project.yaml and on disk.  A prior
    # failed/manual workflow may have left an unregistered iter_NNN directory
    # or YAML behind; reusing it would silently mix stale task_tree nodes into
    # a new formal iteration.
    observed_numbers: list[int] = []
    for value in existing:
        match = _ITER_RE.match(str(value))
        if match:
            observed_numbers.append(int(str(value).split("_", 1)[1]))
    for path in iterations_dir.glob("iter_*"):
        match = _ITER_RE.match(path.stem if path.is_file() else path.name)
        if match:
            observed_numbers.append(int((path.stem if path.is_file() else path.name).split("_", 1)[1]))
    iter_num = max(observed_numbers, default=0) + 1
    ...
    doc = {
        ...
        "implementation_path": (
            str(Path(implementation_path).expanduser().resolve())
            if implementation_path else ""
        ),
    }
