...
def _redact_trace_value(value: Any) -> Any:
    """Redact credentials at the final trace persistence boundary."""
...
class LlmTracer:
    ...
    def _append(self, record: dict) -> None:
        safe_record = dict(_redact_trace_value(record))
        safe_record["ts"] = datetime.now().isoformat()
...
def write_debug_trace(
    ...
    *,
    ...
    try:
        record = dict(_redact_trace_value(record))
