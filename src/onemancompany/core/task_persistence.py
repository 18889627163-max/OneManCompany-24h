    ...
    if projects_dir.exists():
        for tree_path in projects_dir.rglob(TASK_TREE_FILENAME):
            ...
            for node in tree._nodes.values():
                ...
                if formal_v2:
                    # A formal PROCESSING node must be reconciled against its
                    # LangGraph checkpoint before another execution starts.
                    # Rewriting it to PENDING would silently replay the original
                    # task and may duplicate side effects.
                    node.status = TaskPhase.HOLDING.value
                    node.hold_reason = "checkpoint_reconciliation_required"
                    node.hold_started_at = datetime.now().astimezone().isoformat()
                    node.checkpoint_status = "waiting"
                    node.execution_checkpoint = {
                        "phase": "awaiting_checkpoint_reconciliation",
                        "side_effects_confirmed": list(
                            (node.execution_checkpoint or {}).get("side_effects_confirmed", [])
                        ),
                    }
                else:
            ...
            for node in tree._nodes.values():
                ...
                if parent and TaskPhase(parent.status) in RESOLVED:
                    ...
                    if formal_v2:
                        logger.error(
                            "Formal v2 orphan {} remains COMPLETED for controlled reconciliation (parent {} is {})",
