...
def _memory_service_or_503(request: Request):
    ...
    if not settings.omc_memory_enabled or storage is None or storage.memory_store is None:
        raise HTTPException(status_code=503, detail="Long-term memory is unavailable")
    return MemoryService(storage)


def _memory_admin_id(request: Request) -> str:
    from onemancompany.core.memory_service import redact_sensitive

    return redact_sensitive(request.headers.get("X-OMC-Admin-Identity", "admin"))[:100]
...
def _require_runtime_admin(request: Request) -> None:
    """Protect state-changing runtime administration endpoints."""
    from onemancompany.core.config import settings

    if not _is_loopback_request(request):
        raise HTTPException(status_code=403, detail="Runtime administration is loopback-only")
    expected = settings.omc_admin_token
    if not expected:
        raise HTTPException(status_code=503, detail="Runtime administration token is not configured")
    supplied = request.headers.get("X-OMC-Admin-Token", "")
    if not supplied or not secrets.compare_digest(supplied, expected):
        raise HTTPException(status_code=401, detail="Invalid runtime administration token")


async def _runtime_health_payload(request: Request) -> dict:
    """Return sanitized runtime health without paths, credentials or memory data."""
    from onemancompany.core.config import settings

    storage = getattr(request.app.state, "runtime_storage", None)
    gateway = getattr(request.app.state, "provider_gateway", None)
    storage_healthy = bool(storage and await storage.health_check())
    memory_enabled = bool(settings.omc_memory_enabled)
    embedding_status = str(
        getattr(request.app.state, "memory_embedding_status", "disabled" if not memory_enabled else "degraded")
    )
    vector_status = str(
    ...
    provider_metrics = await gateway.metrics() if provider_healthy else {
        "running": 0, "queued": 0, "oldest_queue_at": None
    }
    registration = getattr(request.app.state, "automation_registration", None) or {}
    automation_status = "healthy" if storage_healthy and registration.get("total", 0) >= 13 else ("unavailable" if not storage_healthy else "degraded")
    return {
        "runtime_storage": "healthy" if storage_healthy else "unavailable",
        "checkpoint_store": "healthy" if storage_healthy and storage.checkpointer else "unavailable",
        "memory_store": (
            "disabled" if not memory_enabled
            else "healthy" if storage_healthy and storage.memory_store else "unavailable"
        ),
...
async def runtime_health(request: Request) -> dict:
    return await _runtime_health_payload(request)


@router.get("/api/admin/memories")
async def admin_list_memories(request: Request, status: str = "", scope: str = "", limit: int = 100, offset: int = 0) -> dict:
...
async def admin_get_memory(request: Request, memory_id: str) -> dict:
    _require_runtime_admin(request)
    service = _memory_service_or_503(request)
    try:
...
@router.post("/api/admin/memories/{memory_id}/approve")
async def admin_approve_memory(request: Request, memory_id: str, body: MemoryReviewRequest | None = None) -> dict:
    _require_runtime_admin(request)
    service = _memory_service_or_503(request)
    try:
        return await service.approve(memory_id_or_key=memory_id, admin_id=_memory_admin_id(request), notes=(body.notes if body else ""))
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="Memory not found") from exc
...
@router.post("/api/admin/memories/{memory_id}/reject")
async def admin_reject_memory(request: Request, memory_id: str, body: MemoryReviewRequest | None = None) -> dict:
    _require_runtime_admin(request)
    service = _memory_service_or_503(request)
    try:
        return await service.reject(memory_id_or_key=memory_id, admin_id=_memory_admin_id(request), notes=(body.notes if body else ""))
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="Memory not found") from exc
    except (ValueError, PermissionError) as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
...
@router.post("/api/admin/memories/{memory_id}/supersede")
async def admin_supersede_memory(request: Request, memory_id: str, body: MemoryReviewRequest | None = None) -> dict:
    _require_runtime_admin(request)
    service = _memory_service_or_503(request)
    try:
        return await service.supersede(memory_id_or_key=memory_id, admin_id=_memory_admin_id(request), superseded_by=(body.superseded_by if body else ""), notes=(body.notes if body else ""))
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="Memory not found") from exc
    except (ValueError, PermissionError) as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
...
async def admin_prune_checkpoints(request: Request, older_than_days: int = 30, dry_run: bool = True) -> dict:
    _require_runtime_admin(request)
    storage = getattr(request.app.state, "runtime_storage", None)
    if storage is None or not await storage.health_check():
        raise HTTPException(status_code=503, detail="Runtime storage is unavailable")
    ...
    # Deliberately dry-run only until a formal thread retention policy is approved.
    row = await storage.fetchone("SELECT COUNT(*) FROM checkpoints")
    return {"status": "dry_run" if dry_run else "blocked", "older_than_days": older_than_days, "cutoff": cutoff, "candidate_checkpoints": int(row[0] if row else 0), "message": "Checkpoint deletion requires an approved retention job."}


@router.post("/api/admin/runtime/backup")
async def runtime_backup(request: Request, backup_id: str | None = None) -> dict:
    """Create a transactionally consistent SQLite Online Backup."""
    from onemancompany import __version__
    ...
    if storage is None or not await storage.health_check():
        raise HTTPException(status_code=503, detail="Runtime storage is unavailable")
    backup_dir = Path(settings.omc_runtime_backup_dir)
    if not backup_dir.is_absolute():
        # OMC_DATA_ROOT is the process isolation boundary for runtime data.
        # Relative backup locations must remain inside that boundary rather
        # than leaking into the repository working tree during verification.
        backup_dir = DATA_ROOT / backup_dir
    try:
    ...
    manifest = _json.loads(result.manifest_path.read_text(encoding="utf-8"))
    return {
        "status": "completed",
        "backup_id": manifest["backup_id"],
        "database_file": result.database_path.name,
        ...
        "sqlite_page_count": manifest["sqlite_page_count"],
        "integrity_check": result.integrity_check_result,
    }
...
def _push_adhoc_task(
    ...
    *,
    ...
    if task_key:
        # A non-empty task_key is the explicit marker for standard-v2 durable
        # execution.  The automation runner prepares the SQLite dispatch intent
        # before calling this function and binds the receipt afterwards.
        root.task_key = task_key
        root.dispatch_request_fingerprint = request_fingerprint
...
def _rebuild_employee_agent(employee_id: str) -> bool:
    ...
    loop.agent._agent = create_runtime_react_agent(model=new_llm, tools=tool_registry.get_proxied_tools_for(employee_id))
    return True
...
async def get_bootstrap() -> dict:
    ...
    return {
        "employees": employees,
        "tasks": tasks,
        "active_tasks": [task.to_dict() for task in get_active_tasks()],
        "rooms": rooms,
        "tools": tools,
...
async def ceo_submit_task(
    ...
    if project_id:
        iter_id = create_iteration(
            project_id, task, "pending", implementation_path=implementation_path,
        )
        pid = project_id
    elif project_name:
        pid = create_named_project(project_name, product_id=product_id)
        iter_id = create_iteration(
            pid, task, "pending", implementation_path=implementation_path,
        )
    else:
        pid, iter_id = await async_create_project_from_task(
            task,
            "pending",
    ...
    iteration_doc = load_iteration(pid, iter_id) if iter_id else {}
    authoritative_implementation_path = str((iteration_doc or {}).get("implementation_path", ""))
    ...
    if loop:
        ...
        try:
            ...
            # The authoritative business repository is separate from the
            # internal iteration workspace.  Persist it on the root before
            # creating the EA child so TaskTree.add_child() can inherit it
            # through every later dispatch_child() call.
            ceo_root.implementation_path = authoritative_implementation_path
