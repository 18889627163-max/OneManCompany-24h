...
function normalizeTaskStats(tasks) {
  return (tasks || []).reduce((stats, task) => {
    switch (task.status) {
      case 'processing': stats.running += 1; break;
      case 'pending': stats.queued += 1; break;
      case 'holding': stats.awaiting += 1; break;
      case 'blocked': stats.blocked += 1; break;
      default: break;
    }
    return stats;
...
class AppController {
  ...
  async bootstrap() {
    try {
      ...
      const { employees, tasks, active_tasks, rooms, tools, activity_log, version, office_layout } = data;
      ...
      if (window.officeRenderer) {
        window.officeRenderer.updateState({
          employees, active_tasks: active_tasks || [], meeting_rooms: rooms, tools, office_layout,
        });
      }
      // Show version
      if (version) {
  ...
  async _fetchAndRenderActiveTasks() {
    try {
      const stateData = await fetch('/api/state').then(r => r.json());
      const active_tasks = stateData.active_tasks || [];
      if (window.officeRenderer) {
        window.officeRenderer.updateState({ active_tasks });
      }
  ...
  handleMessage(msg) {
    ...
    if (msg.type === 'state_changed') {
      ...
      if (c.includes('active_tasks') || c.includes('projects')) this._fetchAndRenderActiveTasks();
    ...
    const formatters = {
      ...
      'agent_task_update':   (p) => {
        // Keep the office/dashboard activity contract in sync even when no
        // employee taskboard is open.  The event carries the same TaskNode
        // identity used by /api/state.
        if (p.task && window.officeRenderer) {
          const current = window.officeRenderer.state.active_tasks || [];
          const nodeId = p.task.node_id || p.task.id;
          const next = current.filter(task => (task.node_id || task.id) !== nodeId);
          if (!['finished', 'accepted', 'failed', 'cancelled', 'completed'].includes(p.task.status)) {
            next.push({ ...p.task, node_id: nodeId, current_owner: p.employee_id });
          }
          window.officeRenderer.updateState({ active_tasks: next });
          if (!document.getElementById('dashboard-modal').classList.contains('hidden')) {
            this._renderDashboard();
          }
        }
  ...
  _renderDashboard() {
    ...
    // Calculate stats. Backend exposes domain statuses (processing/pending/
    // holding/blocked); do not compare them to UI-only running/queued values.
    const taskStats = normalizeTaskStats(tasks);
    ...
    content.innerHTML = `
      ...
      <div class="dash-section">
        ...
        <div class="dash-stats">
          <div class="dash-stat"><span class="dash-num">${taskStats.running}</span><span class="dash-label">Running</span></div>
          <div class="dash-stat"><span class="dash-num">${taskStats.queued}</span><span class="dash-label">Queued</span></div>
          <div class="dash-stat"><span class="dash-num">${taskStats.awaiting}</span><span class="dash-label">Awaiting</span></div>
          <div class="dash-stat"><span class="dash-num">${taskStats.blocked}</span><span class="dash-label">Blocked</span></div>
        </div>
      </div>
      <div class="dash-section">
