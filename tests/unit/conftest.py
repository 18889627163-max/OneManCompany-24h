...
def _isolate_disk_writes(tmp_path, monkeypatch):
    """Redirect employee-related runtime paths to a per-test temporary registry.

    This keeps task indexes, workspaces, profiles, and dirty-path detection from
    touching the real ``.onemancompany`` directory during unit tests.
    """
    ...
    for task in list(automation_mod._cron_tasks.values()):
        if hasattr(task, "cancel"):
            task.cancel()
    automation_mod._cron_tasks.clear()
    ...
    monkeypatch.setattr(conversation_mod, "EMPLOYEES_DIR", emp_dir)
    ...
    # Cancel cron jobs while the temporary paths are still patched. Without this
    # teardown, sleeping tasks can wake up minutes later and recreate test
    # employees such as ``e1`` under the real .onemancompany directory.
    for task in list(automation_mod._cron_tasks.values()):
        if hasattr(task, "cancel"):
            task.cancel()
    automation_mod._cron_tasks.clear()
