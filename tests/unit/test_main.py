...
# ---------------------------------------------------------------------------
# Persisted TaskTree startup fence
# ---------------------------------------------------------------------------


class TestRestorePersistedTasksStartupFence:
    def test_disabled_does_not_scan_or_drain(self):
        manager = MagicMock()
        restored = main_mod._restore_persisted_tasks_on_startup(manager, enabled=False)
        assert restored == 0
        manager.restore_persisted_tasks.assert_not_called()
        manager.drain_pending.assert_not_called()

    def test_enabled_restores_and_drains_when_work_exists(self):
        manager = MagicMock()
        manager.restore_persisted_tasks.return_value = 2
        restored = main_mod._restore_persisted_tasks_on_startup(manager, enabled=True)
        assert restored == 2
        manager.restore_persisted_tasks.assert_called_once_with()
        manager.drain_pending.assert_called_once_with()

    def test_enabled_does_not_drain_empty_schedule(self):
        manager = MagicMock()
        manager.restore_persisted_tasks.return_value = 0
        restored = main_mod._restore_persisted_tasks_on_startup(manager, enabled=True)
        assert restored == 0
        manager.restore_persisted_tasks.assert_called_once_with()
        manager.drain_pending.assert_not_called()
