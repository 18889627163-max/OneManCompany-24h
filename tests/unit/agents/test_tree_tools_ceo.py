...
class TestStandaloneCeoRequest:
    ...
    def test_standalone_ceo_request_no_schedule(self, tmp_path):
        """When task_id not in _schedule but target is CEO, creates standalone request.

        The standalone path persists an ad-hoc TaskTree. Keep that write inside
        pytest's temporary directory so a unit test can never append nodes to
        the formal ``.onemancompany`` runtime.
        """
        ...
        try:
            with (
                ...
                patch("onemancompany.core.config.PROJECTS_DIR", tmp_path / "projects"),
            ):
            ...
            assert (tmp_path / "projects" / "_adhoc_ceo" / "task_tree.yaml").is_file()
