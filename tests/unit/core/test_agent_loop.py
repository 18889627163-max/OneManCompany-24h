...
def _make_tree_entry(tmp_path, employee_id="emp01", description="Build widget",
                     project_id="", node_id="", status="pending"):
    ...
    if status != "pending":
        root.status = status
    # Production profile loading is intentionally strict.  Keep this fixture
    # representative of the minimum valid employee contract instead of
    # weakening validation just to support a synthetic employee.
    from onemancompany.core import config as config_mod
    employee_dir = config_mod.EMPLOYEES_DIR / employee_id
