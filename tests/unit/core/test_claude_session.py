...
# ---------------------------------------------------------------------------
# ClaudeDaemon subprocess transport
# ---------------------------------------------------------------------------

class TestClaudeDaemonTransport:
    @pytest.mark.asyncio
    async def test_start_raises_stdout_reader_limit_for_large_ndjson_tool_results(self, tmp_path):
        """A single stream-json tool result may exceed asyncio's 64 KiB default."""
        from onemancompany.core.claude_session import ClaudeDaemon

        daemon = ClaudeDaemon(
            employee_id="00007",
            project_id="project/iter_001",
        ...
        with patch("onemancompany.core.claude_session.EMPLOYEES_DIR", tmp_path), \
             patch("onemancompany.core.claude_session._save_running_pid"), \
             patch.object(daemon, "_drain_initial_stdout", new_callable=AsyncMock), \
             patch("asyncio.create_subprocess_exec", new_callable=AsyncMock, return_value=mock_proc) as spawn:
            await daemon.start()

        reader_limit = spawn.call_args.kwargs.get("limit", 2**16)
        assert reader_limit >= 16 * 1024 * 1024
