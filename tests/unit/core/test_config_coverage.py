...
class TestMemoryEmbeddingDimensionsConfig:
    def test_empty_env_value_means_no_vector_index(self, monkeypatch):
        import onemancompany.core.config as config_mod

        monkeypatch.setenv("OMC_MEMORY_EMBEDDING_DIMENSIONS", "")
        fresh = config_mod.Settings()
        assert fresh.omc_memory_embedding_dimensions is None

    def test_integer_env_value_is_parsed(self, monkeypatch):
        import onemancompany.core.config as config_mod

        monkeypatch.setenv("OMC_MEMORY_EMBEDDING_DIMENSIONS", "1536")
        fresh = config_mod.Settings()
        assert fresh.omc_memory_embedding_dimensions == 1536
...
class TestDataRootOverrideContract:
    def test_data_root_override_is_declared_in_source_and_env_example(self):
        """The process-level isolation boundary must remain a documented contract."""
        from pathlib import Path
        source = Path("src/onemancompany/core/config.py").read_text(encoding="utf-8")
