...
class TestTaskNode:
    ...
    def test_implementation_path_round_trips_in_skeleton(self):
        node = TaskNode(
            employee_id="00007",
            description="formal work",
            implementation_path="/Users/hanzhen/Documents/云测试的项目",
        )

        restored = TaskNode.from_dict(node.to_dict())

        assert restored.implementation_path == "/Users/hanzhen/Documents/云测试的项目"

    def test_from_dict_migrates_in_progress_to_processing(self):
...
class TestTaskTree:
    ...
    def test_dispatch_verification_round_trips_through_yaml(self, tmp_path):
        tree = TaskTree(project_id="proj1")
        root = tree.create_root(employee_id="00001", description="Root")
        child = tree.add_child(root.id, "00010", "Child", ["done"])
        ...
        assert loaded.get_node(child.id).dispatch_verification == child.dispatch_verification
...
class TestWorkflowContractV2:
    def test_new_standard_defaults_to_v2(self):
        tree = TaskTree(project_id="proj-v2", mode="standard")
        assert tree.workflow_contract_version == 2
        assert tree.dispatch_manifest == {}

    def test_new_simple_defaults_to_v1(self):
        tree = TaskTree(project_id="proj-simple", mode="simple")
        assert tree.workflow_contract_version == 1

    def test_historical_tree_without_version_loads_as_v1_without_rewrite(self, tmp_path):
        path = tmp_path / "task_tree.yaml"
        original = "project_id: legacy/iter_001\nmode: standard\nroot_id: ''\nnodes: []\n"
    ...
    def test_v2_fields_round_trip(self, tmp_path):
        tree = TaskTree(project_id="proj/iter_002", mode="standard")
        root = tree.create_root("00004", "root")
        child = tree.add_child(root.id, "00006", "work", ["done"])
        child.task_key = "phase1-smoke"
        child.dispatch_request_fingerprint = "sha256:abc"
        child.execution_generation = 2
        child.checkpoint_thread_id = f"omc:proj:iter_002:{child.id}:g2"
        child.checkpoint_status = "active"
        child.acceptance_audit = {"decision": "accepted"}
        child.review_omission_count = 1
        tree.dispatch_manifest[child.task_key] = {
        ...
        loaded = TaskTree.load(path)
        loaded_child = loaded.get_node(child.id)
        assert loaded.workflow_contract_version == 2
        assert loaded.dispatch_manifest[child.task_key]["node_id"] == child.id
        assert loaded_child.task_key == child.task_key
        assert loaded_child.execution_generation == 2
        assert loaded_child.acceptance_audit == {"decision": "accepted"}
