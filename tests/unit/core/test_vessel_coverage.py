...
class TestRecoverOrphanedTasks:
    ...
    def test_skips_unknown_status_without_crashing_startup(self, tmp_path):
        tree = TaskTree(project_id="p1")
        root = tree.create_root(employee_id="e1", description="root")
        child = tree.add_child(parent_id=root.id, employee_id="e1",
                               description="task", acceptance_criteria=[])
        child.status = "legacy_unknown_status"
        tree_path = tmp_path / "task_tree.yaml"
        tree.save(tree_path)
...
class TestStopAllLoopsCompletion:
    @pytest.mark.asyncio
    async def test_completion_queue_cannot_restart_during_shutdown(self):
        """Late completion callbacks must not recreate the consumer after stop."""
        mgr = EmployeeManager()
        mgr._stopping = True
        entry = ScheduleEntry(node_id="n1", tree_path="/tmp/missing-tree.yaml")

        await mgr._on_child_complete("e1", entry, "p1")

        assert mgr._completion_consumer is None
        assert mgr._completion_queue is None
