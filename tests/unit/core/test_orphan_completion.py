...
def test_recovery_does_not_auto_accept_standard_v2_orphan(tmp_path):
    """Formal v2 recovery must preserve an unreviewed COMPLETED node."""
    tree = TaskTree(project_id="project/iter_010", mode="standard", workflow_contract_version=2)
    parent = tree.create_root(employee_id="00003", description="formal parent")
    parent.task_key = "phase1-parent"
    parent.set_status(TaskPhase.PROCESSING)
    parent.set_status(TaskPhase.COMPLETED)
    parent.set_status(TaskPhase.ACCEPTED)
    parent.set_status(TaskPhase.FINISHED)
    child = tree.add_child(parent.id, "00006", "unreviewed work", [])
    child.task_key = "phase1-smoke-backend"
    child.set_status(TaskPhase.PROCESSING)
    child.set_status(TaskPhase.COMPLETED)

    tree_path = tmp_path / "iter_010" / "task_tree.yaml"
    tree_path.parent.mkdir(parents=True)
    tree.save(tree_path)
    from onemancompany.core.task_tree import _cache
    _cache.clear()

    employee_manager = MagicMock()
    employee_manager.schedule_node = MagicMock()
    from onemancompany.core.task_persistence import recover_schedule_from_trees
    recover_schedule_from_trees(employee_manager, tmp_path, Path("/nonexistent"))

    from onemancompany.core.task_tree import get_tree
    reloaded = get_tree(tree_path)
    assert reloaded.get_node(child.id).status == TaskPhase.COMPLETED.value
    assert reloaded.get_node(child.id).acceptance_audit is None
    _cache.clear()
