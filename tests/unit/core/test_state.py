...
# ---------------------------------------------------------------------------
# get_active_tasks — API contract
# ---------------------------------------------------------------------------

class TestGetActiveTasks:
    def test_filters_non_active_schedule_entries_and_exposes_node_identity(self, tmp_path, monkeypatch):
        import onemancompany.core.state as state_mod
        import onemancompany.core.task_tree as tree_mod
        import onemancompany.core.vessel as vessel_mod

        tree_path = tmp_path / "projects" / "proj" / "iterations" / "iter_005" / "task_tree.yaml"
        tree_path.parent.mkdir(parents=True)
        tree_path.write_text("nodes: []\n", encoding="utf-8")

        statuses = {
            "pending000001": "pending",
            "process00002": "processing",
            "holding00003": "holding",
            "blocked00004": "blocked",
            "complete0005": "completed",
            "accepted0006": "accepted",
            "finished0006": "finished",
            "failed000007": "failed",
            "cancelled008": "cancelled",
        }
        nodes = {}
        for node_id, status in statuses.items():
            node = MagicMock()
            node.id = node_id
            node.parent_id = "parent000001"
            node.project_id = "proj/iter_005"
            node.description = f"task {node_id}"
        ...
        fake_tree = MagicMock()
        fake_tree.get_node.side_effect = nodes.get
        monkeypatch.setattr(tree_mod.TaskTree, "load", lambda _path: fake_tree)
        monkeypatch.setattr(
            vessel_mod.employee_manager,
            "_schedule",
            {
                "00007": [
                    vessel_mod.ScheduleEntry(node_id=node_id, tree_path=str(tree_path))
                    for node_id in statuses
                ]
            },
        )

        tasks = state_mod.get_active_tasks()

        assert {task.status for task in tasks} == {"pending", "processing", "holding", "blocked"}
        assert all(task.node_id for task in tasks)
...
class TestWorkflowV2StatusPolicy:
    def test_parse_task_phase_rejects_empty_unknown_and_wrong_case(self):
        import pytest
        from onemancompany.core.task_lifecycle import parse_task_phase
        for raw in (None, "", "running", "Processing"):
            with pytest.raises(ValueError):
                parse_task_phase(raw)

    def test_parent_and_closure_sets_are_explicit(self):
        from onemancompany.core.task_lifecycle import (
            CLOSURE_COMPLETE, PARENT_TERMINAL_FAILURE, PARENT_VALID, TaskPhase,
        )
        assert TaskPhase.PENDING not in PARENT_VALID
        assert PARENT_TERMINAL_FAILURE == {
            TaskPhase.FAILED, TaskPhase.BLOCKED, TaskPhase.CANCELLED,
        }
        assert CLOSURE_COMPLETE == {TaskPhase.ACCEPTED, TaskPhase.FINISHED}
