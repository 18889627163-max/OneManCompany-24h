...
class TestCreateIteration:
    ...
    def test_iteration_persists_authoritative_implementation_path(self, tmp_path):
        slug = pa.create_named_project("Formal Path")
        expected = tmp_path / "云测试的项目"
        expected.mkdir()

        iter_id = pa.create_iteration(
            slug,
            "formal remediation",
            "COO",
            implementation_path=str(expected),
        )

        doc = pa.load_iteration(slug, iter_id)
        assert doc["implementation_path"] == str(expected.resolve())
...
def test_create_iteration_does_not_reuse_unregistered_iteration_directory(tmp_path):
    slug = pa.create_named_project("Interrupted iteration")
    assert pa.create_iteration(slug, "first", "COO") == "iter_001"
    stale = tmp_path / slug / "iterations" / "iter_002"
    stale.mkdir(parents=True)
    (stale / "task_tree.yaml").write_text("stale: true", encoding="utf-8")

    iter_id = pa.create_iteration(slug, "formal retry", "COO")

    assert iter_id == "iter_003"
    assert (stale / "task_tree.yaml").read_text(encoding="utf-8") == "stale: true"
    assert pa.load_named_project(slug)["iterations"] == ["iter_001", "iter_003"]
