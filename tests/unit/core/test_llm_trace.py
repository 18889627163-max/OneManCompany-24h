...
def test_trace_persistence_redacts_nested_secrets(tmp_path):
    path = tmp_path / "llm_trace.jsonl"
    tracer = LlmTracer(path)
    tracer.log_tool_call(
        ...
        "example",
        {
            "authorization": "Authorization: Bearer abc.def",
            "nested": {"api_key": "api_key=sk-abcdefghijklmnop"},
            "items": ["password=hunter2"],
        },
    )

    payload = path.read_text()
    for secret in ("abc.def", "sk-abcdefghijklmnop", "hunter2"):
