...
def test_debug_trace_redacts_message_and_tool_secrets(tmp_path):
    from langchain_core.messages import AIMessage, HumanMessage

    write_debug_trace(
        str(tmp_path),
        employee_id="00008",
        node_id="secret-filter",
        messages=[
            HumanMessage(content="password=hunter2"),
            AIMessage(
                ...
                tool_calls=[
                    {
                        "id": "call-1",
                        "name": "example",
                        "args": {"api_key": "api_key=sk-abcdefghijklmnop"},
                    }
                ],
            ),
        ],
    ...
    payload = (tmp_path / DEBUG_TRACE_FILENAME).read_text()
    for secret in ("hunter2", "abc.def", "sk-abcdefghijklmnop"):
